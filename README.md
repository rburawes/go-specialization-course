# Golang Course
Coursera - Programming with Google Go Specialization

![Certificate](certificates/Coursera%20SE7LX3ERMEZH.png)
### Getting Started with Go

* [hello.go](getting-started-with-go/hello.go)
* [trunc.go](getting-started-with-go/trunc.go)
* [findian.go](getting-started-with-go/findian.go)
* [makejson.go](getting-started-with-go/makejson.go)
* [read.go](getting-started-with-go/read.go)

#### Certificate

![Certificate](certificates/Coursera%20FGZYQCLTDY8E.png)

### Functions, Methods, and Interfaces in Go

* [bubblesort.go](golang-functions-methods-interfaces/bubblesort.go)
* [displacement.go](golang-functions-methods-interfaces/displacement.go)
* [animals.go](golang-functions-methods-interfaces/animals.go)
* [interfaces.go](golang-functions-methods-interfaces/interfaces.go)

#### Certificate

![Certificate](certificates/Coursera%20T5NTE94T2JEC.png)

### Concurrency in GO

* [Moore's Law](concurrency-in-go/Moore's%20Law.pdf)
* [racecondition.go](concurrency-in-go/racecondition.go)
* [diningphilosophers.go](concurrency-in-go/diningphilosophers.go)

#### Certificate

![Certificate](certificates/Coursera%20Z3ZN2FP6WE9G.png)
